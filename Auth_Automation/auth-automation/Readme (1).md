...
1. **Environment URL:** Update `base.url` to switch between staging and production.  
2. **User Credentials:** Update `valid.email` and `valid.password` for the login flow.  
3. **Reset Link:** The `resetPassword.link` must be updated if testing the password reset completion flow manually, as tokens typically expire.
